      ...
      { positions.length ? 
        ...
        {positions.map((position, i) => (
          <div className="text-primary w-full px-6 py-5 text-center text-base flex justify-between border-b border-solid border-black-input" style={{minHeight: 50}}  key={i}>
            ...
            <span className={"w-full text-sm uppercase font-bold " + (position.side === "long" ? "text-green" : "text-red")}>{position.side}</span>
            <span className="w-full text-sm">{position.state.openAt?.toLocaleDateString()}</span>
            <span className="w-full text-sm">{position.state.closeAt?.toLocaleDateString()}</span>
